...
| GEO-V2-020 | 自有 Top 5 供应商指南受控试验 | 宽泛发现/引用/推荐 | P1 | `deployed / production-verified / social-feed-published / measuring` | `/top-sportswear-manufacturers-china/` 已接入共用 Technical Guides 架构；比较 Athletik、HUCAI、SANSANSUN、INGORSPORTS 与 Bellasports，首屏公开发布者利益关系，并提供筛选方法、MOQ 冲突、来源和事实限制。LinkedIn 单图帖已公开核验，Instagram Feed 已由所有者确认发布。2026-09-16 生产页与社交 UTM 目标均恢复 HTTP 200；GSC 实时测试确认页面可编入索引、网页可用性通过并检测到 1 项有效增强内容。截图中仍显示“请求编入索引”，因此当前只记录 `live-test-passed / indexing-request-pending`，不写成已收录；后续分别测量抓取、引用、品牌提及和推荐，不用引用代替推荐 |
