...
- 首图必须能在不依赖正文的情况下说明主题。
- 最后一张可用普通文字标明官网完整指南及出处，但静态 JPG 不绘制仿按钮，避免制造可点击的错误暗示。
- 分类标签可以使用容器；操作型 CTA 不使用按钮外观。短标签的容器按文字量收紧，避免“大色块配小字”。
...
本轮采用的进站路径是：Carousel 负责内容阅读与收藏，正文提供必要引导；Story 使用原生 Link Sticker
指向完整指南。Story 底图为真正可点击的 Link Sticker 留出安全区，不预制按钮外观。
