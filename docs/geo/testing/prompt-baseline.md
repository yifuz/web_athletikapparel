...
| 2026-09-05 | ChatGPT Search（模型/模式待确认；Sources 面板显示 299） | V2-D03 | 英文回答；Desktop；完整回答和 Sources 面板截图已提供；Temporary Chat、标准 Search/非 Research、Custom Instructions、个性化设置和实际地区待所有者确认 | `partial / environment-and-source-url`：固定原文和第一次回答完整；仍需确认运行模式/环境，并取得 Athletik 来源卡片的实际 URL | Athletik 进入四家短名单第 1 位，被评为 `Very strong`、`cleanest match` 和 `best-supported shortlist`；匹配理由为 ISO 607/Yamato FLATLOCK、Merrow ACTIVESEAM、技术针织品和自有生产设施 | 来源面板可见 Athletik Apparel About Us 卡片，但精确 href 未随截图/粘贴文本保留；规范站引用暂记待确认 | `Athletik Apparel — About Us` 来源卡片；另有两个使用中文名称的 Athletik 历史/矩阵站来源卡片，URL 均待确认 | 主体匹配强但来源混合：当前 About 支持 Yamato FLATLOCK、Merrow ACTIVESEAM、自有 4,500+ m² 设施与技术针织定位；ISO 607 是当前技术指南中的工业参考。回答将这些证据表述为“same technical page”，来源归属不够精确 | 使用 `Zhangjiagang Athletik Clothing Co., Ltd.` 缩写而非核准全称；Athletik 论证混入历史矩阵站。Yonglee 页面所称 `MB-40FD` 与 Merrow 官方 `MB-4DFO` 不一致；Yonglee、Royal、Merino Wool Apparel 的设备数与产能主要为企业自述，第三方资料只部分证明企业/地址，不证明机器当前归属 | Shanghai Yonglee Textile Co., Ltd. / Yonglee Group（第2）；Royal International Industrial Co., Ltd.（第3）；Merino Wool Apparel (Suzhou) Co., Ltd.（第4）；技术来源含 Merrow/ACTIVESEAM，外部佐证含 ISPO 与 D&B | 所有者在 GEO 工作对话提供第一次完整回答和 Sources 面板截图；截图显示 299 个检索来源，仅保存了面板可见部分，未保留 Athletik 卡片实际 URL | 首次运行；强正向推荐信号，待环境与 URL 补证；不标稳定改善 |
...
- Sources 面板可见一个 Athletik Apparel About Us 卡片，以及两个采用中文名称的 Athletik 历史/矩阵站卡片。当前只把 `athletikapparel.com` 作为规范站；历史站可作为检索现象记录，不自动升级为当前事实。需要复制 Athletik Apparel 卡片的实际 URL，才能把本次规范站引用从“可见来源卡片”升级为已核验 URL。
...
- Sources 数量 299 和 46 秒活动时长本身不能判断是否使用了标准 Search 或 Research 模式。所有者需确认此次未使用 Deep Research/Research；若使用了，则保留为探索性观察，不计入24次标准基线。
