...
- [ ] 最终法律审核仍需确认不存在因受控实体、合资、合伙或自愿认证产生的 CCPA 适用性，
      并评估其他适用的美国州隐私法；业务规模变化或启用 Google Ads 前重新筛查。
- [ ] 因公司位于美国但计划主动面向欧洲和英国买家推广，由法律审核判断是否必须指定
      EEA representative 和 UK representative；如适用，在政策中加入其名称和联系方式。
- [x] 2026-08-04 用户确认首轮推广仅面向美国；EEA、英国和瑞士相关代表与跨境事项转为
      区域启用 Gate，未完成前不得主动向这些地区投放。
...
1. ~~用已确认信息形成完整 `privacy-policy-draft.md` 英文审核稿。~~ 已完成。
...
5. 在询盘表单下方加入简短的数据使用提示和 Privacy Policy 链接，不设置必选阅读框；
   如果以后开展邮件营销，再单独评估并提供可选的营销同意。
6. 用全新浏览器复查页面、Cookie Declaration、CMP 分类和实际网络请求。
