...
> 文档对齐日期：2026-08-07
> 规划周期：首个 90 天
> 当前状态：网站、隐私、归因与转化验证已完成；Google 搜索广告与 Instagram 付费推广均已上线；多渠道矩阵总表见 `promotion-matrix.md`
...
2026-08-07：在 SEO 之外新增 GEO（生成式引擎优化）工作流——每篇技术文章按
"定义句 + FAQ + 对比表"格式生产，同时服务 Google 排名和 ChatGPT/Perplexity 等
AI 引擎引用；提示词基线、站外实体建设与月度监测方法见 `promotion-matrix.md`
第 6 节。
...
| 第 31–45 天 | 清除无关词、调整地域、优化落地页映射，并启动目标客户开发 | 可以比较不同来源和产品品类 |
| 第 46–60 天 | 发布首批技术文章和 LinkedIn 证明内容，开始回传线索状态 | 合格和不合格线索得到稳定记录 |
...
- `已确认（2026-08-07）：大部分客户可接受公开展示；单个客户名称或案例发布前仍逐一书面确认。认证与业务事实沿用原有逐项核实规则。`
