...
| 2026-09-15 | 设备图片方案 A 本地实施：不创建独立设备图区块，也不向 7 个产品页加入设备图；在首页 Manufacturing Proof Points 内形成 2×2 能力卡与单张生产配图，在 About 的既有 Difference 区块内形成四卡与一主两辅编辑式拼图，在 Services 的既有 Capabilities 区块内形成单张配图与 2×2 能力卡。5 张源图生成 15 个 480 / 800 / 1536w WebP，存放于 uploads 的 `production/equipment/`；模板补充响应式候选、固有尺寸、lazy/async 与描述性 alt。Desktop / Mobile 均无整页横向溢出，产品页设备引用为 0；URL、Title、Meta、H1、Schema 和现有正文不变，状态 `implemented / owner-review / deployment-pending`，部署时需分别同步主题代码与 uploads 媒体。 |
