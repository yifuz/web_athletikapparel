...
| 2026-09-15 | 设备图片方案 A 收敛为首页单图：仅在首页 Manufacturing Proof Points 内保留 2×2 能力卡与一张生产现场配图，About 与 Services 恢复加入设备图之前的布局，7 个产品页继续保持无设备图。首页保留 1 张源图对应的 480 / 800 / 1536w 三个 WebP 候选，存放于 uploads 的 `production/equipment/`，并保留固有尺寸、lazy/async 与描述性 alt；其余 12 个方案 A 派生 WebP 已从本地 uploads 删除。URL、Title、Meta、H1、Schema 和现有正文不变，状态 `implemented / owner-review / deployment-pending`，部署时需分别同步首页主题代码与这 3 个 uploads 媒体文件。 |
| 2026-09-15 | About / Services 真实生产摄影重新编排：所有者确认 `production/工艺/` 图片均源于真实车间，仅经 ChatGPT 做画面与光线优化，不涉及虚构。About 保留厂房 Hero，在 Our Story 嵌入“工人 + 缝制车间”全景，并将 Who We Serve 配图替换为橄榄绿色成衣缝制近景；Services 保留原 Hero，在 Capabilities 嵌入工人操作图，在 Process 标题区加入裁剪车间图。未创建独立设备图库，也未向 7 个产品页加入设备图；4 张源图生成 12 个 480 / 800 / 1536w WebP，存放于 uploads 的 `production/garment-process/`，模板包含 srcset、sizes、固有尺寸、lazy/async 与描述性 alt。现有正文、URL、Title、Meta、H1 和 Schema 不变，状态 `implemented / owner-review / deployment-pending`。 |
