...
| 2026-09-02 | V1.3 采购决策审计 PCD-001 本地实施并获所有者批准：Fluent Forms Form ID `3` 的预计数量由区间下拉改为 required numeric input，标签简化为 `Estimated Order Quantity`；Tech Pack 改为 required 三状态 Select，并与 Business Type 同行。首页询盘左栏增加四张采购准备卡片，采用暖深灰背景；Desktop 有效内容约 1200px、左右约 35% / 65%、40px 间距，360–2048px 无横向溢出。首页、7 个品类页与 Contact 共 9/9 HTML、客户端空值验证和 `{all_data}` 通知覆盖通过，未执行成功提交。文件上传维持 `deferred`；PCD-002 `no-change`。当前 `ready-to-deploy`，需部署 `style.css`、`template-parts/home/inquiry-cta.php` 并在生产 Form ID `3` 人工同步字段，禁止整库覆盖。 |
