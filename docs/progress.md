...
| 2026-09-02 | V1.3 采购决策审计 PCD-001 本地实施完成：Fluent Forms Form ID `3` 的预计数量由区间下拉改为 required numeric input，标签直接显示 `MOQ 500`；新增 required `Do you have a tech pack?` 三状态字段。首页、7 个品类页与 Contact 共 9/9 HTML、1440×900 / 390×844 视觉、客户端空值验证和 `{all_data}` 通知覆盖通过，未执行成功提交。文件上传因 Fluent Forms Pro 与私有文件治理依赖维持 `deferred`；PCD-002 按所有者决定 `no-change`，网站继续以中国制造主体为主要公开实体。表单配置位于数据库，等待生产 Form ID `3` 后台人工同步，不部署主题文件或整库覆盖。 |
