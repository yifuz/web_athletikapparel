...
> 状态：**未批准新 URL**——本文档为内容简报候选，不授权创建页面
...
> **以下每一项都需要所有者提供真实证据或确认可公开表述后才能写入页面。**
> 标注【NEEDS INPUT】的条目在所有者确认前不执行。

### 4.1 QC 流程节点

【NEEDS INPUT: 确认以下 QC 节点是否真实存在且可公开描述】
...
1. 确认 §4.1 中每个 QC 节点的存在性和可公开程度
2. 确认 §4.2 中可用的图片/素材
3. 确认 §4.3 中的责任边界表述
4. 决策 §4.4 中 Services/About/首页的 "in-house testing" 口径一致性
5. 审核并批准 Title / Meta / H1 草案
6. 批准 URL `/garment-quality-control-checklist/`（或建议其他 slug）
7. 审核完整正文草稿（待证据收集完成后撰写）
...
|---|---|---|
| 1 | 内容简报建立 | ✅ 本文档 |
| 2 | 所有者回答 §4 证据清单 | 待所有者回复 |
| 3 | SERP 验证（US/CA/UK 前 10 结果分析） | 待执行 |
| 4 | 正文草稿撰写 | 待证据齐全后执行 |
| 5 | 所有者审核 | 待草稿完成 |
| 6 | 发布与部署 | 待审核通过 |
| 7 | Day 28 / Day 90 复盘 | 发布后执行 |
