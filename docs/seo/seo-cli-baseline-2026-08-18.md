...
所有者随后提供的 2026-08-28 17:28 GSC“测试实际版本”截图已绿色通过，显示“网址可编入 Google 索引”且网页可用性正常，并确认同日已点击“请求编入索引”一次。该实时证据和平台操作不覆盖此前 indexed snapshot 的 `Discovered - currently not indexed`；当前不重复提交，进入常规复查窗口。
