...
| SEO-V2-005 | 用非品牌 Query 扩充现有页面证据与内链 | IMP-014/022 | P1 | `active / controlled-change` | Knitted Fabrics 与 Underwear 继续按既定窗口测量，不重复改动。2026-09-10 完成 Merino 三个商业词簇 US / GB / CA Top 50 验证；所有者确认 UltraMerino 为同方控制的 Merino 专业站，并批准 Athletik 主站强化 Lightweight Base Layers、Midweight Thermal Programs、Performance Underwear 三个采购入口。当前本地实现待所有者审核与部署 | 不创建 Merino Base Layer / Performance Underwear 平行页，不复制 UltraMerino 内容；部署后先做实际网址测试与一次性索引请求，Day 7 只核验抓取，Day 28 / 90 再依据 Query、有效询盘和双站表现决定 keep / iterate |
