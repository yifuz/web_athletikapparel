...
| SEO-V2-004 | 剩余 LCP 与 Field CWV 复核 | IMP-035–038 | P0 | `deployed / playback-verified / performance-pending` | 2026-09-18 正常 motion 的生产 Desktop / Mobile 均在 load 后请求正确 MP4 并持续播放；强制 reduced-motion 为 0 个 MP4 请求和 poster-only。所有者设备的 Windows 动画状态为关闭，与“不播放”现象一致，因此该现象不是部署或视频资源故障。CrUX Phone 仍 not configured | 保留 reduced-motion 保护；所有者以开启 Windows Animation effects 的正常 motion 环境复核视觉。随后运行同条件三次移动 Lab 和同范围 Crawl Diff；Cookiebot 与共享图片仍为独立变量 |
