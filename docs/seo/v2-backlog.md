...
| SEO-V2-005 | 用非品牌 Query 扩充现有页面证据与内链 | IMP-014/022 | P1 | `conditional` | 2026-08-31 Live SERP 显示 `/underwear-manufacturer/` 在 GB `performance underwear manufacturer` 单次快照为第 10，且 US / GB / CA 意图与页面所有权匹配；仍需等待 GSC 页面/Query 达到流程样本门槛 | Underwear 为第一观察候选，Sportswear Yoga/Activewear 为第二候选；只优化一个主要变量，完成 28 天前后窗口，不创建近义页 |
