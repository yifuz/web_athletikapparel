...
| SEO-V2-003 | 索引、Crawl 与 HTML 响应持续监测 | IMP-033/038 | P0 | `monitoring / owner-action` | 2026-08-28 第二批 8 个 URL Inspection 已完整执行：7 个 indexed / PASS，Services 为 `NEUTRAL / Discovered - currently not indexed`；两批合计 18 个 Sitemap 页面中 17 个 PASS、1 个 Services 待处理。Services 生产端 200、index/follow、自引用 canonical、Sitemap 与首页入口均正常，无代码阻塞证据 | 所有者在 GSC 网页版对 Services 执行“测试实际网址”，通过后只请求编入索引一次；记录结果并继续周期 Crawl / Index Snapshot |
