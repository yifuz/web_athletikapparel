...
| SEO-V2-007 | 品类页社交图 / Schema 主图 | IMP-013 | P1 | `reopened / local-fix-pending-deployment` | 2026-08-27 LinkedIn Post Inspector 新抓取能读取 Title/Description/Canonical，但 WebP `og:image` 显示 `No image found`，已满足原定重开条件。已从 7 张原批准素材制作并视觉检查 1200×627 JPG 兼容图，本地配置已转为 `image/jpeg`，待部署 | 7 页生产 OG/Twitter/Schema 均指向 1200×627 JPG，资源验收通过，LinkedIn Post Inspector 能显示图片且裁切可接受后改为 `fixed / keep` |
| SEO-V2-008 | 五个低曝光品类的响应式图片批次 | IMP-025 | P1 | `local-complete / production-pending` | Sports Accessories 已有 20 曝光、平均排名 6.9，Merino 已有 1 点击；本地已为五页 18 张子类图生成 54 个 480/800/1200w WebP，并补齐 alt、固有尺寸和响应式映射。待部署 `inc/product-category-data.php` 与 uploads 资源后进行生产视觉、MIME、HTML、Lab 与同 URL 审计回归 | 生产 54 个资源全部 200/`image/webp`；18/18 图片输出三档响应式候选、尺寸、lazy/async；视觉、Lab 与定向审计通过并记录最终 outcome |
