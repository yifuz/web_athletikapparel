...
| SEO-V2-004 | 剩余 LCP 与 Field CWV 复核 | IMP-035–038 | P0 | `review / lab-regression` | 2026-09-18 首页视频 Hero 三次同条件移动 Lab LCP 为 4.529 / 4.612 / 4.532s，中位数 4.532s，LCP 节点均为 Hero video；相对 2026-08-27 的 3.66s 基线稳定慢约 0.87s。CLS 为 0、TBT 为 0–140ms；CrUX Phone 仍 not configured | 先对 poster、视频启动时机和 render path 做单变量方案评审，不把 Cookiebot 或共享图片问题混入同一改动；实施后以相同三次移动 Lab 验证，Field 数据继续单独记录 |
