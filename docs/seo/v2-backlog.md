...
| SEO-V2-005 | 用非品牌 Query 扩充现有页面证据与内链 | IMP-014/022 | P1 | `conditional` | 2026-08-31 Live SERP 显示 `/underwear-manufacturer/` 在 GB `performance underwear manufacturer` 单次快照为第 10；Merino 追加验证只有 `custom merino wool clothing manufacturer` 呈现条件式 B2B 意图。GSC 代理恢复后的 final 28 天中，Underwear 合计 2 曝光；Merino 对应 Query 1 曝光 / 排名 23；Outdoor 无可见非品牌 Query 行；Sportswear 唯一行是不匹配的其他公司品牌 | 四页均低于门槛，不改页。Underwear 继续第一观察，Merino 只确认页面所有权，Sportswear 不追逐不匹配品牌；只优化一个主要变量，不创建近义页 |
