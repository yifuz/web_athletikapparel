...
| SEO-V2-015 | 首页 `Technical Knitwear` 定位语义纠偏 | 所有者 Google 截图、业务确认与 US / GB / CA Live SERP | P1 | `implemented / pending-deployment` | 2026-08-31 所有者批准实施；Title/H1 改为 `Performance Apparel Manufacturer`，首屏以 `Performance apparel OEM/ODM partner` 和 knitted fabric / underwear / base layer / sportswear / yoga apparel 业务范围限定意图，不使用 `Cut-and-Sew` 作为首页主定位 | 部署后确认 HTTP 200、Canonical、robots、单一 H1、Title/Meta、OG/Twitter 与 WebPage Schema 一致；重跑 Crawl，并按 Day 7 / 28 / 90 观察 GSC 与有效询盘 |
