...
| SEO-V2-018 | Sportswear 商业词簇与现有页面适配研究 | 90 天 GSC、DataForSEO 关键词估算、公开搜索抽样、现有页审查与所有者业务边界 | P1 | `research / owner-input` | 2026-09-19 GSC 复跑覆盖 2026-06-17 至 09-14：Sportswear 只有 2 条可见 exact-URL Query 行，唯一 eligible 行仍为其他公司名 `sukartik clothing private limited`、1 impression，对应自动 Finding `action-5772d7be34e7` 处置 `not-needed / mismatched-query`，不支持改 Title / Meta / H1。DataForSEO 本轮五词请求因 UTC 日硬限额未执行；公开搜索抽样不是 US / GB / CA exact SERP，但方向上显示 `OEM activewear manufacturer` 反复对应 tech pack、规格审核、打样、批准后批量和 QC，意图与 Athletik 较吻合；`private label sportswear manufacturer` 大量混入低 MOQ、startup 与 teamwear；`technical sportswear manufacturer` 用法稀疏且语义不稳定。现有 Sportswear 页已覆盖 OEM/ODM、activewear、tech pack、500 pieces per style、测试、QC 与四步流程 | 页面所有权暂定：`performance apparel manufacturer` 继续归首页，Sportswear 页承接 sportswear / OEM activewear 的交易型意图，Top 5 指南承接 China supplier comparison。保留现有 URL、Title、Meta、H1，不新建近义页；DataForSEO 额度恢复后补 US / GB / CA 同配置快照。实施前只需确认 Sportswear 的 OEM、ODM、private label 与 white label 实际服务边界，以及 labels / hangtags / packaging 是否可稳定提供；如确认，只评估一个靠近 Buyer Fit 的合作模式/品牌执行正文变量，否则以 `no-change` 完成 |
