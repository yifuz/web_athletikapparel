...
| SEO-V2-003 | 索引、Crawl 与 HTML 响应持续监测 | IMP-033/038 | P0 | `monitoring / request-pending` | 2026-08-28 第二批 8 个 URL Inspection 为 7 个 indexed / PASS、Services `Discovered - currently not indexed`；两批合计 18 个 Sitemap 页面中 17 个 PASS。Services 生产索引信号正常，所有者提供的 17:28 GSC 实时测试已绿色通过，显示“网址可编入 Google 索引” | 所有者点击“请求编入索引”一次并确认；随后不重复提交，在合理抓取窗口内复查 indexed snapshot |
