...
| SEO-V2-002 | 28 天 GSC / GA4 / 询盘基线刷新 | IMP-014/022/033 | P0 | `completed` | 2026-09-08 补齐 2026-08-01 至 08-31 自然月基线：GSC 8 clicks / 329 impressions；GA4 112 sessions，Organic Search 2 sessions / 0 `generate_lead`；所有者最新确认 8 月正式/有效询盘为 0。此前 07-26 至 08-22 的滚动快照保留，但其中将 08-01 记为有效询盘的旧分类已被最新业务核验取代 | 完整自然月数据、样本门槛、归因边界和 `no-change` 决策已记录；GA4 `generate_lead` 不替代人工询盘真值，后续按完整月份和既定 Day 28 / 90 节点比较 |
...
| SEO-V2-005 | 用非品牌 Query 扩充现有页面证据与内链 | IMP-014/022 | P1 | `active / measuring` | Knitted Fabrics 与 Underwear 等待 2026-09-29 Day 28，不重复改动。Merino 生产实现已被 Google 于 2026-09-14 重新抓取并保持 indexed / Canonical 一致；2026-08-17 至 09-13 的 Page 行较前窗增加 4 clicks / 65 impressions，但可见 Query 仅 21 impressions，低于修改门槛 | 保持 Merino URL、Title、Meta、H1 与正文；不创建平行页或复制 UltraMerino 内容。下一步按既定 Day 28 / 90 结合 Query、GA4 与有效询盘决定 keep / iterate，不用匿名 Query 猜词 |
...
| SEO-V2-015 | 首页 `Technical Knitwear` 定位语义纠偏 | 所有者 Google 截图、业务确认与 US / GB / CA Live SERP | P1 | `fixed / measuring` | 2026-09-07 Day 7 通过：Google 于 09-04 重新抓取首页，URL Inspection 为 `PASS / Submitted and indexed`、Canonical 一致；生产 HTML 与当前搜索快照显示新 Title，无明显技术回归。当前最终 GSC 只覆盖三个改版后自然日且 Query 明细不可见，不作效果归因 | 保持当前首页，不重复修改；2026-09-28 做 Day 28 GSC / GA4 / 有效询盘复盘，Day 90 再决定 `keep`、`iterate` 或 `revert` |
...
| SEO-V2-018 | Sportswear 商业词簇与现有页面适配研究 | 90 天 GSC、DataForSEO 关键词估算、公开搜索抽样、现有页审查与所有者业务边界 | P1 | `implemented / owner-review / deployment-pending` | 2026-09-19 GSC 复跑覆盖 2026-06-17 至 09-14：Sportswear 只有 2 条可见 exact-URL Query 行，唯一 eligible 行仍为其他公司名 `sukartik clothing private limited`、1 impression，对应自动 Finding `action-5772d7be34e7` 处置 `not-needed / mismatched-query`，不支持改 Title / Meta / H1。公开搜索抽样显示 `OEM activewear manufacturer` 的规格驱动采购任务与页面较吻合，但不是 US / GB / CA exact SERP；DataForSEO 五词快照仍待 UTC 日额度恢复。所有者确认 Athletik 可按完整 tech pack / reference sample 做 OEM，可按明确概念或不完整规格协助开发型 ODM，可稳定提供 main labels、care labels、hangtags、artwork 与 custom packaging，并明确不提供 ready-stock white label。基于这些一方业务证据，仅在 Sportswear Buyer Fit 后新增合作模式模块；2026-09-22 依据所有者视觉反馈，将三张同风格卡片改为暖灰底、桌面端左右分栏的三条纵向合作路径，Program Fit 的采购筛选信息保持原样。其他六个品类、URL、Title、Meta、H1、图片与 Schema 均不改 | 本地 PHP 语法、7 个品类数据隔离已通过；2026-09-22 生产 HTML 仍未含新增模块，LocalWP 返回 502，真实 Desktop/Mobile 视觉尚不可核验。部署后检查页面 200、单一 H1、三条正文与标题层级、Desktop/Mobile 排版及其他品类无新增模块，再运行定向 Crawl Diff。Finding outcome 为 `changed / owner-review / local-render-unavailable`；上线后进入观察窗口，不因低样本立即继续改写 |

> SEO-V2-018 的表格记录的是合作模式模块当时的实施快照。2026-09-22 所有者另行确认 Sportswear lookbook：最终 14 张真实产品图，首屏 8 张、可拖动并可点击放大；LocalWP 此次已恢复 HTTP 200。随后真实鼠标复验发现并修复画廊过早捕获指针造成的放大失效，Sportswear 鼠标/触控与 Merino 拖动已通过本地复验；仍待生产部署与定向 Crawl Diff。图片属于同一 Sportswear 页面采购核验增强，不新增 Backlog 项，也不改变 URL、Title、Meta、H1 或 Schema。完整边界和 Finding outcome 见 [Sportswear lookbook Change Card](implementation/change-cards/seo-v2-018-sportswear-lookbook.md)。
