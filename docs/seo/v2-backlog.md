...
| SEO-V2-005 | 用非品牌 Query 扩充现有页面证据与内链 | IMP-014/022 | P1 | `conditional` | 2026-08-31 Live SERP 显示 `/underwear-manufacturer/` 在 GB `performance underwear manufacturer` 单次快照为第 10；Merino 追加验证仅有 `custom merino wool clothing manufacturer` 呈现条件式 B2B 意图。同期 GSC 两个报告均因非重试型 `fetch failed` 暂不可用，仍需等待页面/Query 达到流程样本门槛 | Underwear 为第一观察候选，Sportswear Yoga/Activewear 为第二候选；Merino 暂不改页。只优化一个主要变量，完成 28 天前后窗口，不创建近义页 |
