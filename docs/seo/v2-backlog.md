...
| SEO-V2-004 | 剩余 LCP 与 Field CWV 复核 | IMP-035–038 | P0 | `inconclusive / keep-monitoring` | 2026-09-19 部署后三次移动 Lab 的 LCP 中位数为 4.566s，较部署前 4.532s 增加 34ms（约 0.75%），属正常波动；CLS 三次均为 0，LCP 节点已从 Hero video 改为 poster。Crawl Diff Run `2b028751-ef61-4650-bb49-f2c54e2f2b36` 为 0 added / removed / changed / new errors / indexability flips。CrUX Phone 仍 not configured | 保留当前 Hero 和 reduced-motion / Save-Data 防护，不再叠加 Hero 改动；等待可得 CrUX。性能 Finding 转为独立的共享渲染阻塞链与首页图片交付调查，不能把其结果归因到本次 Hero 变量 |
...
| SEO-V2-017 | 首页 Outdoor 类目卡图片交付 | SEO-V2-016 三次移动 Lab 的重复 Outdoor image delivery evidence | P1 | `implemented / deployment-pending` | 三次有效报告均列出约 100 KB 的 Outdoor 1200w q100，估算浪费约 75,565 bytes；2026-09-19 已从同一源图生成 q80 的 480 / 640 / 960 / 1200w 候选，仅更新 Outdoor 一张卡片 | 单独部署模板与 4 个 uploads 图片；生产验证 HTTP、实际候选、Desktop/Mobile 视觉、三次同条件移动 Lab 和 Crawl Diff。Sports Accessories 因所有者既有“不准改动”要求保持 `deferred / owner-protected`，本批不触碰 |
