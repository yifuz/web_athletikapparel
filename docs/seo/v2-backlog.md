...
| SEO-V2-004 | 剩余 LCP 与 Field CWV 复核 | IMP-035–038 | P0 | `implemented / deployment-pending` | 2026-09-18 首页视频 Hero 三次同条件移动 Lab LCP 中位数为 4.532s，较 2026-08-27 的 3.66s 基线稳定慢约 0.87s；现已把既有 poster 恢复为独立 eager/high 首屏图，MP4 改为 `window.load` 后 idle 注入，并恢复 reduced-motion、Save-Data、后台和离屏控制。CrUX Phone 仍 not configured | 部署 `template-parts/home/hero.php`、`functions.php`、`style.css`、`assets/js/home-hero-video.js` 后，先做 Desktop/Mobile 与请求顺序验收，再运行同条件三次移动 Lab 和同范围 Crawl Diff；Cookiebot 与共享图片仍为独立变量 |
