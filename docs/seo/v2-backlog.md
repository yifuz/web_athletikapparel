...
| SEO-V2-007 | 品类页社交图 / Schema 主图 | IMP-013 | P1 | `production-accepted / LinkedIn-preview-pending` | 2026-08-28 JPG 兼容修复已部署；7 页生产 OG/Twitter/Schema、7 个 JPG 资源、Title/Meta/H1/Canonical 和定向审计全部通过。仍缺 LinkedIn Post Inspector 实际重新抓取结果 | LinkedIn Post Inspector 能显示图片且裁切可接受后改为 `fixed / keep`；若仍为 `No image found`，转查 bot 回源、缓存和 Cloudflare，不盲目换图 |
