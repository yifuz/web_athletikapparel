...
| SEO-IMP-037 | 优化全站首屏阻塞链：Manrope、Fluent Forms CSS、jQuery、WP Consent API、Cookiebot 与 CSS minify | 中到高：影响多模板 FCP/LCP | 中到高：Consent、GA4、表单和插件依赖不可破坏 | **本地实施完成（2026-08-25）**：Manrope 改为主题内 Latin/Latin-ext WOFF2 自托管，移除 Google Fonts CSS/字体外链与 preconnect，并只预加载首屏 Latin 文件；Fluent Forms CSS/jQuery 已按表单页加载，Consent/Cookiebot 保持早期加载，CSS minify 因收益低保持不改。PHP、HTML、字体文件与 Desktop 截图验收通过；待生产验收。见 [`Change Card`](change-cards/seo-imp-037-first-paint-blocking-chain.md) |
| SEO-IMP-038 | 核实 Flywheel / Cloudflare HTML 页面缓存策略并建立慢响应升级阈值 | 中：可能改善 HTML 交付稳定性与抓取体验 | 中到高：错误页面缓存会影响登录、Consent、表单或个性化响应 | **监测基线完成（2026-08-25）**：Cloudflare HTML 为 `DYNAMIC`，但 Flywheel/Fastly 返回 `x-cacheable: YES` 与 `MISS, HIT`，页面缓存有效；静态 CSS 为 Cloudflare HIT。建立 5 URL × 3 轮脚本及 normal/review/host-escalation-ready 阈值，不执行 Cache Everything 或缓存配置变更。见 [`Change Card`](change-cards/seo-imp-038-html-cache-response-monitoring.md) |
